...
class Maze:
    ...
    def reset(self) -> None:
        """Initializes or resets the maze structure to its default 'all-walls-intact' state."""
        ...
        # Track visited cells for both generation and solving phases
        ...
        # Start the generation at a random interior cell
    ...
    def cell_has_all_walls(self, row: int, col: int) -> bool:
        """Helper to check if a cell is still completely enclosed by walls."""
    ...
    def in_bounds(self, row: int, col: int) -> bool:
        """Checks if a given (row, col) coordinate is within the maze boundaries."""
        return 0 <= row < self.rows and 0 <= col < self.cols

    def generation_neighbors(self, row: int, col: int) -> list[Cell]:
        """Finds adjacent cells that haven't been visited yet during generation."""
        candidates: list[Cell] = []
        # Check all four cardinal directions (North, South, West, East)
        for d_row, d_col in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if self.generation_visited[next_row][next_col]:
                continue
            # A candidate neighbor must still have all its walls intact
    ...
    def carve_between(self, current: Cell, neighbor: Cell, is_extra: bool = False) -> None:
        ...
        # Determine which wall to remove based on the relative position of the neighbor
        if next_row == row - 1:
        ...
        # Update statistics for tracking maze complexity
        if is_extra:
    ...
    def finish_generation(self) -> None:
        """Finalizes the maze by adding cycles (if bonus) and setting entrances."""
    ...
    def generate_step(self) -> bool:
        """Executes one step of the DFS-based maze generation algorithm."""
    ...
    def add_extra_openings(self) -> None:
        """Randomly removes additional walls to create cycles in the maze (bonus mode)."""
        for row in range(self.rows):
            for col in range(self.cols):
                # Try removing horizontal walls between rows
                ...
                # Try removing vertical walls between columns
    ...
    def select_entrances(self) -> None:
        """Selects start and end points for the maze, opening boundary walls if necessary."""
        if self.bonus_mode and self.rows > 2 and self.cols > 2:
            # For bonus mode, start and end are randomly placed in the maze interior
        ...
        # Default mode: entrance on the left, exit on the right
        ...
        # Open the boundary walls for entrance and exit
        self.east_wall[start_row][0] = 0
        self.east_wall[end_row][self.cols] = 0

    def can_move(self, current: Cell, neighbor: Cell) -> bool:
        """Determines if there is a passage (no wall) between two adjacent cells."""
        row, col = current
        next_row, next_col = neighbor
        ...
        # Check if the wall between current and neighbor has been removed
    ...
    def solve_step(self) -> bool:
        """Executes one step of the recursive backtracking maze solver."""
        if self.solver_complete:
            return True

        if not self.solve_stack:
            # Entire maze explored, no path found
        ...
        if current == self.end_cell:
            # Success! Copy the stack to get the final solution path
        ...
        if options:
            # Advance to the next unvisited cell
            chosen = self.rng.choice(options)
...
class MazeWindow:
    ...
    def draw(self) -> None:
        """Renders the current state of the maze using OpenGL primitives."""
        glClear(GL_COLOR_BUFFER_BIT)
        _, _, cell_size = self.grid_geometry()

        # 1. Draw background fills for start and end cells
        ...
        # 5. Draw the active solver "mouse" cursor
        ...
        # 6. Draw all horizontal and vertical walls
        x0, y0, cell_size = self.grid_geometry()
        wall_width = max(2.0, cell_size * 0.08)

        # Draw horizontal (north) walls
...
def main() -> None:
    """Entry point for the maze application. Parses arguments and launches the window."""
    parser = build_parser()
    args = parser.parse_args()
    
    # Set default extra wall chance for bonus mode if not specified
    ...
    # Initialize the core maze logic
    maze = Maze(args.rows, args.cols, args.extra_wall_chance, args.seed, bonus_mode=args.bonus)
    
    # Choose graphical backend based on availability
    if GRAPHICS_READY:
        # Preferred: PyOpenGL/pygame
        window = MazeWindow(
            maze=maze,
            width=args.width,
            height=args.height,
